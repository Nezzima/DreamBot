...
public class Fisher extends AbstractScript {
	...
	Hashtable<Tile, Tile> poolHash = new Hashtable<>();
	Filter<NPC> poolFilter = n -> {
		if (n == null)// || !n.exists())
			return false;
	...
	private boolean needToStop() {
		if (!Inventory.contains(sv.yourFish.getItemName()))
			return true;
		return !sv.yourFish.getRequiredItem().equals("") && !Inventory.contains(sv.yourFish.getRequiredItem());
	}
	...
	public void onPaint(Graphics g) {
		if (started) {
			...
			g.drawString("Runtime: " + timer.formatTime(), 5, 105);
			...
			for (int i = 0; i < lootTrack.size(); i++) {
				...
				if (p != null && p.getAmount() > 0) {
					g.drawString(p.getName() + "(p/h):" + p.getAmount() + "(" + timer.getHourlyRate(p.getAmount()) + ")", 5, 135 + place * 15);
					place++;
				}
			}
			if (state != null)
				g.drawString("State: " + state, 5, 135 + place * 15);
